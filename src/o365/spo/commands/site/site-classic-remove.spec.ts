...
describe(commands.SITE_CLASSIC_REMOVE, () => {
  ...
  it('logs correct telemetry event', (done) => {
    ...
    cmdInstance.action({ options: {} }, () => {
      try {
        assert.equal(telemetry.name, commands.SITE_CLASSIC_REMOVE);
        done();
      }
      catch (e) {
        done(e);
  ...
  it('aborts removing site when prompt not confirmed', (done) => {
    ...
    cmdInstance.action({ options: { url: 'https://contoso.sharepoint.com/sites/project-x', debug: true, verbose: true } }, () => {
      try {
  ...
  it('removes site. doesn\'t wait for completion', (done) => {
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { url: 'https://contoso.sharepoint.com/sites/project-x', confirm: true, debug: true, verbose: true } }, () => {
      try {
        // not called?
        assert(cmdInstanceLogSpy.called);
        done();
  ...
  it('removes site, skip recyclebin doesn\'t wait for completion', (done) => {
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { url: 'https://contoso.sharepoint.com/sites/project-x', confirm: true, debug: true, verbose: true, skipRecycleBin: true } }, () => {
      try {
        // not called?
        assert(cmdInstanceLogSpy.called);
        done();
  ...
  it('removes site from recyclebin doesn\'t wait for completion', (done) => {
    sinon.stub(request, 'post').callsFake((opts) => {
      if (opts.url.indexOf(`/_vti_bin/client.svc/ProcessQuery`) > -1) {
        if (opts.headers.authorization &&
          ...
          opts.headers['X-RequestDigest'] &&
          opts.headers['X-RequestDigest'] === 'abc' &&
          opts.body === `<Request AddExpandoFieldTypeSuffix="true" SchemaVersion="15.0.0.0" LibraryVersion="16.0.0.0" ApplicationName="${config.applicationName}" xmlns="http://schemas.microsoft.com/sharepoint/clientquery/2009"><Actions><ObjectPath Id="185" ObjectPathId="184" /><Query Id="186" ObjectPathId="184"><Query SelectAllProperties="false"><Properties><Property Name="IsComplete" ScalarProperty="true" /><Property Name="PollingInterval" ScalarProperty="true" /></Properties></Query></Query></Actions><ObjectPaths><Method Id="184" ParentId="175" Name="RemoveDeletedSite"><Parameters><Parameter Type="String">https://contoso.sharepoint.com/sites/project-x</Parameter></Parameters></Method><Constructor Id="175" TypeId="{268004ae-ef6b-4e9b-8425-127220d84719}" /></ObjectPaths></Request>`) {
          return Promise.resolve(JSON.stringify([
            {
              "SchemaVersion": "15.0.0.0", "LibraryVersion": "16.0.8015.1210", "ErrorInfo": null, "TraceCorrelationId": "5eda879e-90d5-6000-d611-e6bfd5acde9f"
            }, 12, {
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { url: 'https://contoso.sharepoint.com/sites/project-x', fromRecycleBin: true, confirm: true, debug: true, verbose: true } }, () => {
      try {
        // not called?
        assert(cmdInstanceLogSpy.called);
        done();
  ...
  it('removes site. wait for completion', (done) => {
          ...
          return Promise.resolve(JSON.stringify([
            {
              ...
              "_ObjectType_": "Microsoft.Online.SharePoint.TenantAdministration.SpoOperation", "_ObjectIdentity_": "5eda879e-90d5-6000-d611-e6bfd5acde9f|908bed80-a04a-4433-b4a0-883d9847d110:2ca3eaa5-140f-4175-9563-1172edf9f339\nSpoOperation\nRemoveSite\n636707032254311675\nhttps%3a%2f%2fdigiwijs.sharepoint.com%2fsites%2ftodelete1\n445b4444-2a90-4794-8912-77891fb9fd02", "IsComplete": false, "PollingInterval": 15000
            }
          ]));
        }
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { url: 'https://contoso.sharepoint.com/sites/project-x', confirm: true, debug: true, verbose: true, wait: true } }, () => {
      try {
        // not called?
        assert(cmdInstanceLogSpy.called);
        done();
  ...
  it('removes site, skip recyclebin wait for completion', (done) => {
    sinon.stub(request, 'post').callsFake((opts) => {
      if (opts.url.indexOf(`/_vti_bin/client.svc/ProcessQuery`) > -1) {
        if (opts.headers.authorization &&
          opts.headers.authorization.indexOf('Bearer ') === 0 &&
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { url: 'https://contoso.sharepoint.com/sites/project-x', confirm: true, debug: true, verbose: true, wait: true, skipRecycleBin: true } }, () => {
      try {
        // not called?
        assert(cmdInstanceLogSpy.called);
        done();
  ...
  it('removes site from recyclebin wait for completion', (done) => {
    sinon.stub(request, 'post').callsFake((opts) => {
      if (opts.url.indexOf(`/_vti_bin/client.svc/ProcessQuery`) > -1) {
        if (opts.headers.authorization &&
          opts.headers.authorization.indexOf('Bearer ') === 0 &&
    ...
    auth.site.tenantId = 'abc';
    cmdInstance.action = command.action();
    cmdInstance.action({ options: { url: 'https://contoso.sharepoint.com/sites/project-x', fromRecycleBin: true, confirm: true, debug: true, verbose: true, wait: true } }, () => {
      try {
        // not called?
        assert(cmdInstanceLogSpy.called);
        done();
